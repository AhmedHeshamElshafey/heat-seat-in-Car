...
extern uint32 ulTimeIn[7];
extern uint32 ulTimeOut[7];
extern uint32 ulExecutionTime[7];

#define traceTASK_SWITCHED_IN()   \
        uint32 TaskInTag= (uint32) (pxCurrentTCB->pxTaskTag); \
ulTimeIn[TaskInTag]=GPTM_WTimer0Read();

#define traceTASK_SWITCHED_OUT()  \
        uint32 TaskoutTag= (uint32) (pxCurrentTCB->pxTaskTag); \
ulTimeOut[TaskoutTag]=GPTM_WTimer0Read(); \
ulExecutionTime[TaskoutTag]+=ulTimeOut[TaskoutTag]-ulTimeIn[TaskoutTag];
